...
本文从工程实践出发，区分系统的部署方式与代码组织方式，并选取三个常见组合——模块化 MVC 三层单体、细粒度多仓微服务 MVC、模块化单体 DDD——分析它们在 AI 编码语境下的表现差异。通过代码示例，本文进一步论证显式边界与分层约束如何影响生成行为、降低审阅成本，并给出轻量级落地建议。
...
| 适用场景 | 跨模块协作简单，规则可以集中在应用服务中的系统 | 确有独立扩缩容、故障隔离或团队自治需求的系统 | 多个入口需要遵守同一规则、上下文语义不同，但暂不需要独立部署的系统 |
...
#### 3.5.5 为什么本文重点讨论 Vibe Coding + DDD

本文并不是说 Vibe Coding 必须使用 DDD。场景一用模块化三层更直接，场景三也必须优先满足独立运行需求。本文重点讨论 DDD，是因为当 AI 成为主要编码者后，真正容易失控的往往不是代码写不出来，而是同一规则被生成到多个入口、同一术语被解释成不同对象，或外部接口变化时顺手改乱业务逻辑。

DDD 为这些问题提供了 AI 能直接读取和遵循的代码结构：

- 限界上下文和业务术语说明一个概念属于哪里。
- 领域方法为状态变更提供固定入口，减少重复生成规则。
- Port 和 infra 把外部接口变化限制在适配代码中。
- 类型、测试和依赖检查让部分漏改与越界在本地暴露。

这些做法对人工开发同样有用。Vibe Coding 带来的变化是代码生成更快、次数更多，一旦规则只靠口头约定，错误写法也会被更快复制；把约束写进模型、接口和检查规则，才能让生成速度不超过团队的验证能力。这正是本文把 Vibe Coding 与 DDD 放在一起讨论的原因。

**选择顺序**：需要独立扩缩容、故障隔离或自治发布，先选择微服务，再决定服务内部使用三层还是 DDD；不需要独立部署时，规则能集中在少量应用服务中就选择组合 A，多个入口需要遵守同一规则或上下文语义不同时选择组合 C；如果多个服务长期一起修改和发布，应先减少服务数量。

**第 3 章结论**：部署方式按运行需求决定，代码组织按业务规则决定。不要为了缩小 AI 的修改范围而拆出大量服务，也不要为了使用 DDD 而制造过多限界上下文；只有当明确的模型、规则入口和依赖方向能够实际减少生成偏差与审阅成本时，这些结构才有价值。
