...
- 业务规则通常集中在 service；业务简单时足够清晰，规则增多后 service 容易膨胀。
- MVC 三层本身不负责划分业务领域。是否按业务模块组织目录、统一术语，需要团队另行设计。
...
**适用场景**：CRUD 为主、业务规则较少或变化较稳定的系统。它并不限于小工具和原型；只要业务模块、接口、依赖方向和测试保持清晰，也可以支撑长期维护。随着术语冲突、状态规则和跨模块协作增加，可以逐步引入领域模块、业务不变量与限界上下文，而不必为了采用 DDD 一次性重写系统。
