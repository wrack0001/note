...
|------|----------------|----------------------|--------------------|
| 定义 | 单仓单部署，业务模块内采用三层 | 多仓多部署，服务内 MVC | 单仓单部署，按限界上下文组织 |
| AI 主要收益 | 模块范围明确、一次构建 | 工作空间统一检索、服务级验证 | 模型归属清楚、规则落点稳定 |
| 主要成本 | 跨模块耦合依赖约定治理 | 多仓、Proto 与发布协调 | 建模成本、统一构建与发布 |
| 验证方式 | 类型、模块接口、测试 | 契约、集成测试、部署检查 | 类型、Port、架构测试 |
| 适合 | 模块清楚、语义稳定 | 需独立运行与团队自治 | 规则复杂、上下文语义不同 |
| 不适合 | 模型泄漏、跨模块规则复杂 | 服务爆炸、跨仓改动频繁 | 简单 CRUD 或强运行隔离需求 |
