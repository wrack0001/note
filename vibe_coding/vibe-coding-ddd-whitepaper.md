...
本文讨论的新项目有一个常见特征：会员、订单、资产、通知等模块各自有少量业务规则，不是纯 CRUD，却也没有重到需要独立部署。如果每个模块都拆成服务，会很快增加仓库、契约和发布成本；如果只用三层把它们放在一起，规则归属和跨模块调用又容易依赖人工约定。
...
**定义与成立前提**：一个仓库、一个部署单元，代码先按会员、订单、支付等业务模块组织，每个模块内部再采用 `handler → application service → repository` 三层。一个业务模块可以很大，内部可以包含多个应用服务；它不是一个独立部署的微服务。跨模块协作通过对方公开的模块接口完成。
...
```
flowchart LR
    S1["Service A<br/>会员"] -.-> S2["Service B<br/>资产"]
    S2 -.-> S3["Service C<br/>通知"]
```
...
- **跨仓修改不是原子变更**：工作空间可以同时编辑多个仓库，但每个仓库仍有独立分支、提交、CI 和合并状态；例如“开通会员→更新资产→发送通知”少合并一个消费者或配置仓库，仍会漏改。
...
**定义与成立前提**：系统仍放在一个仓库中并整体部署，但代码按会员、订单、支付等限界上下文组织，每个上下文拥有自己的模型和业务规则。上下文内部可以采用四层：interfaces 接收请求并转换数据，application 组织业务流程，domain 保存业务模型与规则，infra 负责数据库和外部系统接入。关键不在目录名称，而在业务规则留在 domain，外部依赖通过 Port 接入。
...
例如会员后台的“标签管理”，主要是新增、编辑、删除和查询，规则只有名称必填、不能重复等简单校验，也没有多条状态流转。此时采用模块化 MVC 三层即可：handler 接收请求，应用服务完成校验和流程，Repository 读写数据库。
...
同样，当会员上下文中的“用户”表示登录账号，而 CRM 团队口中的“用户”其实是签约客户时，也应让两个上下文维护各自的模型，通过公开契约转换语义，避免 AI 把同一个词直接映射成同一种对象。
...
例如“开通会员→创建资产账户→通知设备→推送消息”几乎每次变化都要修改四个仓库，而且这些服务没有明显不同的流量、故障隔离或发布节奏。此时问题不是工作空间能否同时打开四个仓库，而是一次需求仍要分别完成契约升级、合并、验证和发布。这通常说明服务拆得过细。

可以先合并为一个部署单元：规则简单时使用组合 A；多个入口需要共同遵守规则，或不同模块对同一概念有不同含义时，使用组合 C。反过来，DDD 的限界上下文也不能按功能菜单无限细分；login、register、profile 如果共享同一套模型和生命周期，就没有必要分别建成三个上下文。

#### 3.5.5 为什么 Vibe Coding 新项目更适合从组合 C 开始

这类新项目通常同时包含会员、订单、资产、通知等多个模块。单看每个模块，业务逻辑都不算重；放到一起看，它们又各自有术语、状态和少量规则。为每个模块建立服务，会过早承担多仓、契约和发布成本；全部使用三层也能开发，但随着模块增多，规则归属和跨模块调用会越来越依赖团队约定。

组合 C 提供了中间路线：先保持一个仓库和部署单元，再用限界上下文划分模块。简单模块可以保持轻量；有状态规则的模块再把规则放进领域模型；数据库和外部调用通过 Port 交给 infra。它不要求新项目第一天就上齐聚合、领域事件和 CQRS。

Vibe Coding 让这个起点更重要，因为 AI 会沿着已有代码快速扩展。项目最早形成的目录、接口和调用方式，很快就会被后续生成反复复用。轻量 DDD 可以给 AI 和人工审核提供同一套明确规则：

- 限界上下文和业务术语说明代码应该放在哪个模块。
- 领域方法为状态变更提供固定入口，避免每个新入口重新生成规则。
- Port 和 infra 把数据库、RPC 与消息处理留在适配代码中。
- 四层职责为人工审核提供固定检查项，类型、测试和依赖检查则提前暴露部分漏改。

这些规则对人工开发同样有用。Vibe Coding 的不同之处在于生成更快、修改更频繁：结构清楚，正确写法会被反复复用；结构含糊，错误写法也会被更快复制。新项目从组合 C 开始，是为了在代码大量生成之前先固定业务归属和修改入口。

**默认选择与例外**：

- 项目整体只是简单 CRUD，选择组合 A。
- 项目包含多个轻量业务模块，每个模块都有自己的术语或少量规则，优先从组合 C 开始。
- 某个模块确有独立扩缩容、故障隔离或自治发布需求，再把它拆成服务；服务内部仍可使用 DDD。
- 多个服务长期一起修改和发布，说明拆分收益不足，应先减少服务数量。
