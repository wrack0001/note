...
本文从工程实践出发，区分系统的部署方式与代码组织方式，并选取三个常见组合——全局 MVC 分层单体、多仓微服务 MVC、模块化单体 DDD——分析它们在 AI 编码语境下的表现差异。通过代码示例，本文进一步论证显式边界与分层约束如何影响生成行为、降低审阅成本，并给出轻量级落地建议。
...
- **组合 A：全局 MVC 分层的单体 / 大仓**——一个部署单元，整个代码库主要按 `handler → service → DB` 横向分层，没有进一步划分业务模块接口。
...
本文所说的“全局 MVC 三层”，特指 Web 后端中常见的 `handler/controller → service → repository/DB` 横向技术分层，而不是狭义的界面 MVC 模式。这个组合只有一个仓库和部署单元，整个代码库主要按协议处理、业务流程与数据访问组织目录，却没有在每一层内继续划分稳定的业务模块接口。它在规模较小时结构直观，但随着领域和团队增多，横向层会逐渐变成共享修改区。

**这种组合自身的工程取舍：**

- 技术职责直观，团队容易理解 `handler → service → repository/DB` 的调用方向。
- 业务规则通常集中在全局 service 层；业务简单时足够清晰，领域增多后容易出现大型 service、循环依赖和跨领域调用。
- 同一个业务变更可能同时穿过全局 handler、service、repository 三个目录，相关代码按技术类型分散，改动的局部性会随着规模增长而下降。
- MVC 三层本身不负责划分业务领域。是否按业务模块组织目录、统一术语和设计模块接口，需要团队另行完成。
- Repository、外部客户端接口、依赖注入和自动化测试都可以正常使用；service 直绑 ORM、RPC 或直接修改数据库，并不是三层架构的必然结果。

**AI 编码带来的增量影响：**

AI 与人工开发面对的是同一套架构约束。不同之处主要在生成速度、改动规模和可用上下文：

- AI 会优先沿用代码库中已经可见的结构。清晰的模块接口和测试会被复用，耦合、复制等坏模式也可能被更快复制。
- 当业务规则分散在多个 service、配置和调用方中时，人需要在脑中拼接上下文，AI 则需要把相关文件检索进当前上下文；信息缺失时，二者都会漏掉约束。
- 代码生成变快后，测试、审阅和架构检查如果没有同步提速，原有问题会以更高频率进入代码库。

**适用场景**：规模较小、CRUD 为主、业务规则较少或变化稳定，且主要由一个团队维护的系统。

**不适合场景**：包含多个业务领域、多人并行开发、规则持续增长的复杂单体或大仓。如果整个代码库仍只有一套全局 handler、service、repository 目录，技术分层无法提供足够的模块接口和改动局部性，AI 与人工都需要在大范围代码中拼接业务上下文。

这里不适合大仓的是“全局横向分层”，而不是单体部署或 MVC 本身。大型单体仍然可以先按业务模块组织，再在每个模块内部采用三层架构；随着术语冲突、业务不变量和跨模块协作增加，也可以逐步引入限界上下文与领域模型，而不必一次性重写系统。
...
这个组合把系统按功能拆成多个仓库、进程和部署单元，每个服务可以独立发布与扩缩容；但每个服务内部仍采用全局 MVC 技术分层。这里讨论的局限来自“多仓部署 + 服务内贫血 MVC”这一具体组合，而不是说所有微服务都会如此。微服务内部如果按业务模块组织或采用 DDD，同样可以建立清晰的领域模型和限界上下文。
...
| 维度 | A：全局分层单体 | B：多仓微服务 MVC | C：模块化单体 DDD |
|------|-------------|--------------------|--------------------|
| 部署方式 | 一个仓库、一个部署单元 | 多个仓库、进程与部署单元 | 一个仓库、一个部署单元 |
| 领域边界 | 需要另行按业务模块划分，技术分层本身不表达领域 | 进程隔离明确，但服务内部的语义边界取决于内部设计 | 以限界上下文和 `domain/<name>` 显式划分 |
| 改动落点 | 取决于模块接口与规则是否集中，三层调用方向本身不限制业务改动面 | 一次需求可能跨多仓、多发布 | 同领域有明确落点；跨领域走接口 |
| 依赖方向 | 可以通过 Repository 与客户端接口隔离，也可能被实现为直接依赖 | 服务互调需要治理，否则可能成环 | domain 定义 Port，infra 实现 |
| 协议与业务 | 分层目标是隔离二者，实际效果取决于 handler 是否保持轻量 | 各服务分别维护协议与内部模型 | interfaces 只做协议↔DTO |
| 适配与业务 | 可以设置接口与 Adapter，但不是该模式的默认要求 | 每个服务分别维护外部适配 | 对方能力差异收口 infra；domain 只表达意图 |
| 人工审阅 | 结构熟悉、易上手；规则分散后需要跨多个 service 核对 | 跨仓审阅需要同时核对契约与版本 | 可按层核对：domain 无框架、infra 不承载业务决策 |
| AI / 自动化防漏 | 与人工相同，依赖类型、测试、静态检查与评审 | 单仓构建无法覆盖完整跨服务契约 | 配合接口、依赖检查与架构测试，可在本地发现部分越界与漏改 |
| AI 改动粒度 | 取决于模块深度；大而浅的 service 容易产生较宽的 diff | 一次需求跨多仓时需要分步协调 | 可按层分面（Port→infra→domain），控制单次 diff |

**一句话总结**：AI 没有改变软件架构的基本问题，但提高了代码生成速度，也放大了既有结构的反馈效应。结构清晰时，正确模式更容易被复用；结构混乱时，耦合与重复也会扩散得更快。
