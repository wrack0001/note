...
| 判断条件 | 更合适的方向 |
|----------|--------------|
| 业务简单、规则稳定 | 从组合 A 开始，避免提前建模 |
| 规则复杂、跨模块修改频繁 | 选择组合 C，先收口语义与业务不变量 |
| 必须独立扩缩容、隔离故障或自治发布 | 选择组合 B，或把组合 C 中对应上下文拆成服务 |
| Proto、契约测试和多仓发布工具链不成熟 | 优先减少服务数量，避免把协调成本交给人工 |
| 单仓构建、发布或团队协作已成为明确瓶颈 | 依据限界上下文拆分部署单元，而不是按代码行数拆 |

领域与服务都不是越细越好。只有当概念含义、业务规则、生命周期或运行需求确实独立时才拆；代码量大、目录拥挤或预想未来需求，都不足以单独构成拆分理由。

**第 3 章结论**：先选择满足当前需求的最简单结构。Vibe Coding 不改变架构规律，只会放大现有结构的反馈效应；清晰的模块接口、可执行约束与匹配业务的粒度，比仓库或服务数量更重要。
