...
- **上下文集中且有范围**：完整模型位于同仓，限界上下文又为一次修改提供明确的领域范围。
- **规则和适配各有落点**：业务不变量收口在 domain，Port 隔离 ORM、RPC 和外部协议，减少生成代码顺手改乱业务的机会。
- **更多问题能在本地暴露**：类型、模块接口和架构测试可以提前发现字段漏改、Port 未实现和部分依赖越界。

**Vibe Coding 中的劣势**：

- **边界不会自动生效**：只有目录还不够，仍需包可见性、依赖检查和架构测试阻止跨模块调用。
